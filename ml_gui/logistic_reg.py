...
class UI(QMainWindow):
    ...
    def training(self):
        ...
        self.lr = LogisticRegression(C=float(self.c_.text()),penalty=self.penalty.currentText(),dual=self.dual.currentText()=='True',tol=float(self.tol.text()),max_iter=float(self.max_iter),fit_intercept=self.fit_inter.currentText()=='True',random_state=1,solver=self.solver.currentText(),multi_class=self.multi_class.currentText())
